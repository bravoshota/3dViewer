...
std::string ascStl2Off(char * filename)
{
	// input variables:
    ...
    std::string offFile(filename, 0, std::strlen(filename) - 3);
    offFile += "off";
...
std::string binStl2Off(char * filename)
{
	// input variables:
	// filename - the path of file to load
...
int isLoop(uint32_t start, uint32_t vsize, uint32_t esize, uint32_t * edgeArr, int32_t * segArr)
{
	...
	for (uint32_t i = 0; i < vsize; i++)
		...
		// and -1 for others
            ...
            IndexMap[i] = static_cast<GLuint>(-1);
	...
	while (buf.size() > 0)
	{
		...
		for (uint32_t i = 0; i < esize * 2; i++)
			...
			if (edgeArr[2 * i] == cur) {
                ...
                if (IndexMap[edgeArr[2 * i + 1]] == static_cast<GLuint>(-1)) {
					// set the distance
