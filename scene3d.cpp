	...
	if (seg3.size() > 0) {
		...
		for (int j = 0; j < fmesh; j++)
		{
			...
			for (int v = 0; v < 3; v++) {
                ...
                if (SkelSegVert[skl] == static_cast<int>(seg3[0]) && skl != static_cast<int>(seg3[1]) && skl != static_cast<int>(seg3[2])) isSeg++;
				else notSeg = v;
                if (skl == static_cast<int>(seg3[2]) || skl == static_cast<int>(seg3[3])) isEnd = 1;
			}
